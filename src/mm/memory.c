 ...
 */
static inline void copy_cow_page(struct page * from, struct page * to, unsigned long address)	//写时复制
{
	if (from == ZERO_PAGE(address)) {	//页面空
...
static inline void free_one_pgd(pgd_t * dir)	//释放页全局目录
{
	...
	pgd_clear(dir);	//清目录
	//该循环为了释放PMD全部映射关系
	for (j = 0; j < PTRS_PER_PMD ; j++) {	//PTRS_PER_PMD是PMD的表项个数
 ...
 */
void clear_page_tables(struct mm_struct *mm, unsigned long first, int nr)	//释放一个范围呢的页表
{
			...
			struct vm_area_struct *vma)
{
	...
	for (;;) {//死循环
		...
		src_pgd++; dst_pgd++;	//下一位置
		
		/* copy_pmd_range */
		
		if (pgd_none(*src_pgd))	//资源不存在
			goto skip_copy_pmd_range;	//跳过复制
		if (pgd_bad(*src_pgd)) {	//坏页
			pgd_ERROR(*src_pgd);
			pgd_clear(src_pgd);
//？？？？？？？？？底下这个公式真的不太懂 感觉就是强行加地址并提前跳出
skip_copy_pmd_range:	address = (address + PGDIR_SIZE) & PGDIR_MASK;	//PGDIR_SIZE页目录表中一个表项 (pgd_t)对应的虚拟地址空间
		...
		do {
			...
			if (pmd_bad(*src_pmd)) {
				pmd_ERROR(*src_pmd);
				pmd_clear(src_pmd);
skip_copy_pte_range:		address = (address + PMD_SIZE) & PMD_MASK;	//道理同上
				if (address >= end)
					goto out;
			...
			do {
				...
				if (cow && pte_write(pte)) {	//写时复制
					ptep_set_wrprotect(src_pte);	。。同时保护父子
					pte = *src_pte;	
				}
...
cont_copy_pte_range:		set_pte(dst_pte, pte);	//进行复制
...
static inline int zap_pte_range(mmu_gather_t *tlb, pmd_t * pmd, unsigned long address, unsigned long size)	//释放页
{
	...
	size &= PAGE_MASK;	//到第几页？
	for (offset=0; offset < size; ptep++, offset += PAGE_SIZE) {	//清页循环直到size
		pte_t pte = *ptep;
 ...
 */
static struct page * follow_page(struct mm_struct *mm, unsigned long address, int write) //快速查页
{
	pgd_t *pgd;
	pmd_t *pmd;
	pte_t *ptep, pte;
...
//取得用户页面，这个函数没太懂
{
	...
	do {
		...
		do {
			struct page *map;
			while (!(map = follow_page(mm, start, write))) {	//找到页面
				spin_unlock(&mm->page_table_lock);	//解锁
				switch (handle_mm_fault(mm, vma, start, write)) {	//缺页中断？
				case 1:
					tsk->min_flt++;
					break;
				case 2:
...
//用户写多个进程共用的页时，需要申请一个新页，然后把旧页的数据复制过来，修改页表项
{
	...
	if (!TryLockPage(old_page)) {	//判断能否加锁
	 ...
	 */
	spin_lock(&mm->page_table_lock);	//上锁
	if (pte_same(*page_table, pte)) {	//目录是否被改写
...
static void vmtruncate_list(struct vm_area_struct *mpnt, unsigned long pgoff)
{
	do {
		...
		if (mpnt->vm_pgoff >= pgoff) {	//虚拟区域偏移量大于期望
	...
	int write_access, pte_t * pte)
{
	...
	//请求调页
	entry = *pte;
	if (!pte_present(entry)) {	//再次认证该页面当前不在内存
