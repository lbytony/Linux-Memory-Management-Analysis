 ...
 */
 /*
	第一步是通过slab队列头中的指针firstnotfull
	找到队列中第一个含有空闲对象的slab
	如果这个指针指向slab队列的第一个slab
		表示队列没有含有空闲对象的slab
		则转到__kmem_cache_alloc的alloc_new_slab处，进一步扩充slab队列
	如果找到含有空闲空间的slab
		调用kmem_cache_alloc_one_tail分配一个空闲对象
 */
...
try_again:
	...
	return objp;
alloc_new_slab:		//没有空闲对象，只能分配新slab
#ifdef CONFIG_SMP
	spin_unlock(&cachep->spinlock);
